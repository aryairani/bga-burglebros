...
var Burgle = (function() {
  ...
  // zz3 - create a floor (an array of [index][n|e|s|w] => true if unblocked)
  var to_floor = function(walls) {
    var i = 0,
      x = 0;
    var floor = new Array(size_sq + 1)
      .join(1)
      .split("")
      .map(function() {
        return { heat: 0 };
  ...
  // zz4 - check path is unblocked
  var valid = function(floor) {
    var check = [shaft == 0 ? 1 : 0];
    var visited = 0;
    // consider the shaft tile already visited
    if (shaft > -1) {
      floor[shaft].v = true;
      visited++;
    }
    ...
    while (check.length > 0) {
      var next = check.pop();
      var tile = floor[next];
      if (tile.v) continue;       // do not visit twice
      visited++;
      tile.v = true;
      if (tile.n) check.push(next - size);  // up
      if (tile.e) check.push(next + 1);     // right
      if (tile.s) check.push(next + size);  // bottom
      if (tile.w) check.push(next - 1);     // left
    }
    // could visit all the tiles of the square --> return true
  ...
  var build_distance = function(floor) {
    var i,
      dist = new Array(size_sq * size_sq + 1)
        .join(1)
        .split("")
        .map(function() {
          return 50;
  ...
  // Function called to generate walls and floors
  // Parameters are 3:4:8 (floors:size:walls)
  var generate = function(id) {
    var floors;
    var max = 2 * size * (size - 1);  // max tiles per floor && max walls per floor
    var permanent_walls = [];         // static walls (around shaft)
    var shaft_walls = [];
    if (id === undefined || id === "all") {
      floors = document.getElementsByClassName("floor");
      if (size === 5) {
        if (shaft > -1) { // reset class of previous shafts if exists
          for (var f = 0; f < floors.length; f++) {
    ...
    // Shaft is a black tile used on Fort Knox to handle 5 x 5 square
    if (shaft > -1) {
      shaft_walls = get_walls(shaft); // return an array of values of each wall position
      shaft_walls.forEach(function(w) {
        permanent_walls[w] = true;
      });
    }
    // zz1 - generation of a random set of walls
    for (var f = 0; f < floors.length; f++) {
      var id = floors[f].getAttribute("id");
      while (true) {
        var wall = permanent_walls.slice(); // copy into a new array (and reset if while is called again)
        for (var w = 0; w < walls; ) {  // walls is in the option the number of walls to setup
          // Create a random wall
          var n = Math.floor(Math.random() * max);
          if (!wall[n]) { // check a permanent wall doesn't already exists
            w++;
            wall[n] = true;
          }
        }
        ...
        if (set_layout(id, wall)) {   // set_layout validate the created walls
          break;
        }
      }
    }
  ...
  // Get all the walls around a tile
  // Return an array of 0 to 4 values of all the wall indexes around a tile
  var get_walls = function(tile) {
    var dec = size - 1,     
      max = 2 * size * dec, // last tile (or last wall)
      off = tile % size,    // offset of column
      row = Math.floor(tile / size) * (size + dec),
      ind = row + off,      // index of tile
      val = [];
    if (off > 0) val.push(ind - 1); // wall on the left
    if (off < dec) val.push(ind);   // wall on the right
    if (ind >= size + dec) val.push(ind - size);  // wall on top
    if (ind + dec < max) val.push(ind + dec);     // wall on bottom
    return val;
  };

  /* Functions to update DOM */
  // Set dimentions and variables
  // Shaft is worth 12 if job == 2
