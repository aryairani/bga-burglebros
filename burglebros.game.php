...
require_once("modules/Notify.class.php");
    ...
    */
    ...
    public function activeNextPlayerCustom() {
        ...
        if ($multi_characters > 1) {
            ...
            $this->notifs->activatePlayer($next_player_id);
...
SQL;
            ...
            do {
                ...
                if ($count == 0) {
                    ...
                    if ($die_count < 6) {
                        ...
                        $this->notifs->message(clienttranslate('The Patrol Deck ran out of cards. The Guard on floor ${floor} now moves ${next_count} spaces'), [
                            'floor' => $floor,
                            'next_count' => $next_count,
                        ]);
                        $this->notifs->patrolDieIncreased($die_count + 1, array_values($this->tokensOfType(TokenType::Patrol, $floor))[0], $floor);
                    }
                }
                $patrol_entrance = $this->cards->pickCardForLocation($patrol->deckName(), $patrol->discardName(), 16 - $count);
                $this->notifs->nextPatrol($floor, $this->cards->getCardsInLocation($patrol->discardName()), $patrol_entrance, $this->cards->countCardInLocation($patrol->deckName()));
    ...
    function flipTile($floor, $location_arg) {
        ...
        $this->notifs->tileFlipped($this->findTileOnFloor($floor, $location_arg), $floor, $this->stateValue(GameStateValue::UndoAllowed), $this->getFlippedTiles());
    }
    ...
    function performGuardMovementEffects($guard_token, $tile_id, $create_path = FALSE) {
        ...
        if ($create_path) {
            $this->notifs->createGuardPath($floor, $this->getPathByLocation($floor, null));
        } else {
            $this->notifs->updateGuardPath($floor, $this->getPathByLocation($floor, null), $tile['location_arg']);
        }
    ...
    function decrementPlayerStealth($player_id, $amount = 1) {
        ...
        $this->notifs->stealthChanged($amount > 0, $players[$player_id]['player_name'], $token_id);
    }
...
SQL;
    ...
    function notifyPlayerHand($player_id, $discard_ids=array()) {
        $this->notifs->playerHand($player_id, $this->cards->getPlayerHand($player_id), $discard_ids);
    }

    function notifyTileCards($tile_id) {
        $this->notifs->tileCards($tile_id, $this->getCardTokens($tile_id));
    }
    ...
    function applyDieRoll($rolls=null, $safe_tile=null, $drop_loot=0) {
        ...
        if ($cracked_count - 1 == ($size_sq - 1) * 2) { // remove a safe cracked count
            ...
            $this->notifs->message(clienttranslate('${player_name} cracked the safe on floor ${floor}'), [
            ...
            for ($lower_floor=$floor; $lower_floor >= 1; $lower_floor--) { 
                ...
                if ($die_count < 6) {
                    ...
                    $this->notifs->message(clienttranslate('Guard on floor ${lower_floor} now moves ${next_count} spaces'), [
                        'lower_floor' => $lower_floor,
                        'next_count' => $next_count,
                    ]);
                    $this->notifs->patrolDieIncreased($die_count + 1, array_values($this->tokensOfType(TokenType::Patrol, $lower_floor))[0], $lower_floor);
                }
            }
            // Show back current cracked safe floor (because patrolDieIncreased will show the 1st floor)
            $this->notifs->showFloor($this->tileFloor($safe_tile), false);
        }
    }
...
SQL;
    ...
    function moveTokens($ids, $location, $location_arg=0, $synchronous=FALSE) {
        ...
        $this->notifs->tokensPicked($synchronous, $this->getTokens($ids));
    }
    ...
    function notifyRoll($rolls, $for) {
        ...
        $this->notifs->diceRolled($this->getActivePlayerNameCustom(), $roll_list, $for);
    }
...
SQL;
    ...
    function triggerAlarm($tile, $skip_token_checks=FALSE, $skip_hacker_checks=FALSE) {
        ...
        $this->notifs->message(clienttranslate( 'An alarm was triggered' ), array());
    ...
    function handleSelectCardChoice($card, $selected_type, $selected_ids) {
        ...
        if ($type == 'acrobat1') {
            ...
            $this->notifs->removeWall($selected_id);
        } elseif($type == 'go-with-your-gut') {
            $this->validateSelection('tile', $selected_type);
            $tile = $this->tiles->getCard($selected_id);
            $flipped = $this->getFlippedTiles($this->tileFloor($tile));
            ...
            $this->notifs->message(clienttranslate('Crystal Ball: ${player_name} changed order of upcoming events to ${card_names_displayed}'), [
                'player_name' => self::getCurrentPlayerName(),
            ...
            $this->notifs->message(clienttranslate('Stethoscope: ${player_name} changed one die from ${old_value} to ${new_value}'), [
                'player_name' => self::getCurrentPlayerName(),
                'old_value' => $old_value,
                'new_value' => $new_value
            ]);
    ...
    function handleSelectTileChoice($selected) {
        ...
        if ($selected == 0) { // trigger
            ...
            $this->notifs->message(clienttranslate('${player_name} used a hack token'), [
                'player_name' => self::getCurrentPlayerName()
            ]);
            ...
            $this->notifs->message(clienttranslate('${player_name} used an extra action'), [
                'player_name' => self::getCurrentPlayerName()
            ]);
        }
        return array(
    ...
    function performAddSafeDie($tile) {
        ...
        $this->notifs->safeDieIncreased($die_num, $safe_token, $floor);
        $this->notifs->message(clienttranslate('${player_name} added a die to the safe on floor ${floor}'), [
            'player_name' => self::getCurrentPlayerName(),
            'floor' => $floor
        ]);
    }
...
SQL;
    ...
    function handleSelectPlayerChoice($current_player_id, $type, $selected) {
        if ($type == 'trade') {
            ...
            $players = $this->loadPlayersInfos();
            $patrol_names = $this->patrolNames();
            $this->notifs->message(clienttranslate('The Rook Advanced: ${player_name} (${rook_tile_name} on floor ${rook_tile_floor}) trades places with ${other_player_name} (${other_tile_name} on floor ${other_tile_floor})'), [
    ...
    function handleSelectSpecialChoice($type, $choice_arg, $selected) {
        if ($type == 'rook1') {
            ...
            $this->notifs->message(clienttranslate('The Rook: ${player_name} wants to move ${other_name}'), [
    ...
    function switchTilesDebug($floor_1, $location_arg_1, $floor_2, $location_arg_2) {
        ...
        $this->notifs->tileFlipped($this->findTileOnFloor($floor_1, $location_arg_1), $floor_1, $this->stateValue(GameStateValue::UndoAllowed));
        $this->notifs->tileFlipped($this->findTileOnFloor($floor_2, $location_arg_2), $floor_2, $this->stateValue(GameStateValue::UndoAllowed));
    }
    ...
    */
    function randomizeWalls(string $floor) {
        if ($floor === 'start') {
            ...
            $this->notifs->updateWalls($msg, $floor, $this->getWalls(), $tiles);
        }
    }
    ...
    function hack() {
        ...
        $this->notifs->message(clienttranslate('${player_name} added a hack token'), [
            'player_name' => self::getCurrentPlayerName()
        ]);
        $this->endAction();
    }
    ...
    function playCard($card_id) {
        ...
        if ($card['type'] == CardType::Character->value) {
            ...
            $this->notifs->characterChosen(self::getCurrentPlayerName(), $current_player_id, $character, $this->getDisplayedCardName($this->getCardType($card)));
            ...
            if ($choice) {
                ...
                $this->notifyPlayerHand($current_player_id, array($card['id']));
                $type = $this->getCardType($card);
                $this->notifs->message(clienttranslate('${player_name} played the ${title} card (${tooltip})'), [
                    'i18n' => ['title', 'tooltip'],
                    'card_id' => $card['id'],
                    'player_name' => self::getCurrentPlayerName(),
    ...
    function selectCardChoice($type, $id, $check_action = TRUE) {
        ...
        if ($this->stateValue(GameStateValue::StealthDepleted)) {
            ...
            if ($card['type'] == CardType::Event->value) {
                ...
                $type = $this->getCardType($card);
                $this->notifs->message(clienttranslate('${player_name} used their character action'), [
                    'player_name' => self::getCurrentPlayerName()
                ]);
    ...
    function proposeTrade($p1_ids, $p2_ids) {
        ...
        // $players = self::loadPlayersBasicInfos();
        $players = $this->loadPlayersInfos();
        $this->notifs->message(clienttranslate('${player_name} proposed a trade to ${other_name}'), [
    ...
    function confirmTrade($force = FALSE) {
        ...
        // $players = self::loadPlayersBasicInfos();
        $players = $this->loadPlayersInfos();
        $this->notifs->message(clienttranslate('${player_name} agreed to ${current_name}\'s trade'), [
    ...
    function cancelTrade() {
        ...
        if ($stateName == 'confirmTrade') {
            $this->notifs->message(clienttranslate('${player_name} cancelled the trade'), [
                'player_name' => self::getActivePlayerName()
            ]);
            $this->gamestate->nextState('endTradeOtherPlayer');
        } else {
    ...
    function confirmTakeCards($l_ids, $r_ids) {
        ...
        $this->notifs->message(clienttranslate('${player_name} picked up ${card_count} cards in their tile'), [
    ...
    function confirmRookMove() {
        ...
        if ($this->getSoloMultiCharacters() > 1) {
            ...
            $current_player_id =  self::getCurrentPlayerId();
            $this->notifs->message(clienttranslate('${player_name} accepts the move'), [
                'player_name' => self::getActivePlayerName(),
            ]);
    ...
    function cancelRookMove() {
        ...
        $this->notifs->message(clienttranslate('${player_name} cancels the move'), [
            'player_name' => self::getActivePlayerName(),
        ]);
    ...
    function pickUpCat() {
        ...
        $this->notifs->catPicked($this->getActivePlayerNameCustom());
        $this->endAction(0);
    }
    ...
    function pass() {
        ...
        if ($actions_remaining >= $trigger_action_count) {
            ...
            if ($event_card) {
                $this->notifs->eventCard($event_card, $this->getCardTitle($event_card), $this->getCardTooltip($event_card));
    ...
    function escape() {
        ...
        $this->notifs->playerEscape($current_player_id, $this->getActivePlayerNameCustom(), $player_token['id']);
...
////////////
    ...
    */
    ...
    function stEndAction() {
        ...
        if ($draw_tools_player_id == 0) {
            ...
            if ($drop_loot > 0 && $drop_loot == $draw_tools_player_id) {
                ...
                $drawer_name = isset($players[$draw_tools_player_id]) ? $players[$draw_tools_player_id]['player_name'] : $players[$current_player_id]['player_name'];
                $this->notifs->addTooltipToLog($drawer_name, $card, $this->getDisplayedCardName($card_name), $this->getCardTooltip($card));
    ...
    function stEndTurn() {
        ...
        $this->resetGlobalVars();
        $this->notifs->message(clienttranslate('${player_name} ended their turn'), [
            'player_name' => $this->getActivePlayerNameCustom()
        ]);
        if ($special_choice) {
            $this->setStateValue(GameStateValue::SpecialChoice, SpecialChoice::ClosestAlarm);
    ...
    function stNextPlayer() {
        ...
        if ($jump_the_gun) {
            ...
            $this->notifs->message(clienttranslate( 'Skipped ${player_name}\'s turn' ), array(
                'player_name' => $players[$player_id]['player_name'],
            ));
        ...
        if ($persian_kitty) {
            if (count($this->getPlacedTokens(array(TokenType::Cat))) == 0) {
                ...
                if (isset($rolls[1]) || isset($rolls[2])) {
                    ...
                    $this->notifs->catEscaped($this->getActivePlayerNameCustom());
                }
            }
        }

        $player_tile = $this->getPlayerTile($player_id);
        $this->notifs->showFloor($this->tileFloor($player_tile), true);
    ...
    function stGameOver() {
        ...
        foreach ($tiles_unflipped as $tile_id => $tile) {
            $this->notifs->tileFlipped($tile, $this->tileFloor($tile), 0, $this->getFlippedTiles());
        }
